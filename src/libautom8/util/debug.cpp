...
sigslot::signal3<debug::debug_level, std::string, std::string> debug::string_logged;
...
namespace autom8 {
    ...
    public:
        struct log_entry {
            log_entry(debug::debug_level l, const std::string& t, const std::string& m) {
                level_ = l;
                tag_ = t;
                message_ = m;
            }

            debug::debug_level level_;
            std::string tag_;
            std::string message_;
        };
...
void debug::init() {
    std::unique_lock<decltype(system_mutex_)> lock(system_mutex_);
